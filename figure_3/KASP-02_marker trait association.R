######################################################################### 
#### Script for statistical analysis and plotting of data (figure 3) ####
####															 
...
####
####
#### Dataset: rqtl_HR_BC1S1_full_population.csv
####
...
### 1) Load data
data = read.cross(format = "csv", dir = ".",  file = "rqtl_HR_BC1S1_full_population.csv", crosstype = "f2", estimate.map = TRUE)

summary(data)
plot(data)

### 2.1) Calculate marker segregation distortion
# NOTE: NOT USED. Marker distortion was instead tested with Chi-square test, see file "fig.3a.xlsx"
gt <- geno.table(data)
gt
...
data = read.cross(format = "csv", dir = ".",  file = "rqtl_HR_BC1S1_full_population.csv", crosstype = "f2")
...
lod_score
  ...
  #write.table(lod_score, file = "lod_score_BC1-S1.txt") # FINAL RESULTS for Sup Table S2
